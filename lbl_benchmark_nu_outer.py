...
@cuda.jit
def cuLBL_calcu(Sw, Gamma_D, Gamma_L, Nu, wave_start, wave_end, wing_cut, sampling_delta, unit_k, sample_point, absor_coef, coef):
    """
    ...
    :param absor_coef: Output absorption coefficient
    """
    ib = cuda.blockIdx.x
    it = cuda.threadIdx.x
    ibd = cuda.blockDim.x
    ...
    gridStride = igd * ibd
    for idx in range(idxWithinGrid, len(Nu), gridStride):
        for i in range(len(sample_point)):
            if wave_start <= sample_point[i] <= wave_end:
                delta_nu = sample_point[i] - Nu[idx]
                if abs(delta_nu) < wing_cut:
                    cuda.atomic.add(coef, i, unit_k * Sw[idx] * voigt_profile(delta_nu, Gamma_D[idx], Gamma_L[idx]))
...
if __name__ == '__main__':
    ...
    g1 = np.array([32, 64, 128, 256, 512, 1024])
    g2 = np.array([6, 5, 4, 3, 2, 1])
    results = np.zeros((6,6))
    for bi, bs in enumerate(g1):
        for gi, gs in enumerate(g2):
            threads_per_block = bs
            blocks_per_grid = int((len(Nu) + threads_per_block - 1) / threads_per_block)
            grid_stride = gs
            ...
            cuLBL_calcu[blocks_per_grid, threads_per_block](Sw_gpu, Gamma_D_gpu, Gamma_L_gpu, Nu_gpu, wave_start,
                                                            wave_end,
                                                            wing_cut, sampling_delta, unit_k, sample_point_gpu,
                                                            absor_coef_gpu,
                                                            coef_gpu)
            cuda.synchronize()
            gpu_calcu_time = time()
            print(f"GPU run cost: {gpu_calcu_time - data_prepare_time}")

            cost_time = np.zeros(10)
            for i in range(len(cost_time)):
                ...
                cuLBL_calcu[blocks_per_grid, threads_per_block](Sw_gpu, Gamma_D_gpu, Gamma_L_gpu, Nu_gpu, wave_start,
                                                                wave_end,
                                                                wing_cut,
                                                                sampling_delta, unit_k, sample_point_gpu,
                                                                absor_coef_gpu,
                                                                coef_gpu)
                cuda.synchronize()
            ...
            results[gi, bi] = np.mean(cost_time)
            savemat("a2509_cuLBL_1_parasAdjustment_0.001.mat", {"results": results})