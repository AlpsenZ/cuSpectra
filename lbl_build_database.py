...
if __name__ == '__main__':
    ...
    nP = int((P_range[1] - P_range[0]) // dP + 1)
    ...
    for i in range(nP):
        for j in range(nT):
            ...
            Coef_data[i * nT + j, :] = output_coef
            gpu_calcu_end_time = time()
            cost_time[i * nT + j] = gpu_calcu_end_time - gpu_calcu_start_time
            print(f"GPU run {i * nT + j} cost: {cost_time[i * nT + j]}")
        savemat(f'a2509_data_coef_{i}.mat', {'Pc': Pdata, 'Tc': Tdata, 'Coef': Coef_data})
        print(f"a2509_data_coef_{i}.mat saved. P: {P_range[0] + i * dP}")
