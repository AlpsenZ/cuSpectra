...
# Parallelized by non-uniform computation points along the path
...
def gpu_fs(P, T, norms, paras, out_spec, N):
    ...
    for idx in range(idxWithinGrid, len(P), gridStride):
        for i in range(N):
            out_spec[idx,i] = surf_fit(P[idx], T[idx], norms, paras, i)
...
nu = data['nu'].squeeze()
P = np.random.uniform(0.1000001, 1.99999999, pt_num)
T = np.random.uniform(250.000001, 1249.9999999, pt_num)
...
PP = np.random.uniform(0.1000001, 1.99999999, pt_num)
TT = np.random.uniform(250.000001, 1249.9999999, pt_num)
P_g = cuda.to_device(PP)
T_g = cuda.to_device(TT)
out_g = cuda.to_device(np.zeros((len(PP), s_channel)))
...
for bi, bs in enumerate(g1):
    for gi, gs in enumerate(g2):
        ...
        for i in range(len(cost_time)):
            ...
            gpu_fs[blocks_per_grid, threads_per_block](P_g, T_g, norms_g, paras_g, out_g, s_channel)
            cuda.synchronize()
            cost_time[i] = time() - begin_time
        print(f"#-> GPU run average cost: {np.mean(cost_time)}")
        results[gi, bi] = np.mean(cost_time)
savemat("a2509_cuUSAR_fastspec_parasAdjustment_10.mat", {"results": results})
